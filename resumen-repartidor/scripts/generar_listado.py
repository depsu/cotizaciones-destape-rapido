...
ESTILOS_EXTRA = """
  ...
  /* Velo de carga (2026-09-02): el HTML trae un respaldo horneado que puede estar
     VIEJO (clientes de hace semanas). Antes se revelaba a los 1.5-2.2 s aunque
     Supabase no hubiera respondido, y por unos instantes se veían entregas antiguas
     y un contador de pendientes falso. Ahora: esqueleto mientras carga, y se revela
     recién cuando llegan los datos vivos (o el caché de la última visita). El
     fallback CSS a los 10 s existe solo por si el JS muere. */
  main { opacity:0; transition:opacity .3s ease; animation:velo-fallback 0s linear 10s forwards; }
  main.listo { opacity:1; }
  @keyframes velo-fallback { to { opacity:1; } }
  body.listo #skel { display:none; }
  #skel { max-width:560px; margin:0 auto; padding:14px 12px 0; }
  .skel-tabs { display:flex; gap:8px; margin-bottom:14px; }
  .skel-tab { flex:1 1 0; height:46px; border-radius:11px; }
  .skel-tit { height:14px; width:42%; margin:16px 4px 10px; border-radius:7px; }
  .skel-card { height:118px; border-radius:14px; margin-bottom:10px; }
  .skel, .skel-tab, .skel-tit, .skel-card { background:linear-gradient(90deg,#E2E8F0 25%,#EDF2F7 45%,#E2E8F0 65%);
    background-size:400% 100%; animation:skel-shimmer 1.3s ease-in-out infinite; }
  header .skel { display:inline-block; vertical-align:middle; height:11px; width:64px; border-radius:6px;
    background:linear-gradient(90deg,rgba(255,255,255,.28) 25%,rgba(255,255,255,.5) 45%,rgba(255,255,255,.28) 65%);
    background-size:400% 100%; }
  header .skel.skel-n { width:16px; }
  header .skel.skel-w { width:150px; height:13px; }
  @keyframes skel-shimmer { 0% { background-position:100% 0; } 100% { background-position:0 0; } }
  @media (prefers-reduced-motion: reduce) { .skel, .skel-tab, .skel-tit, .skel-card { animation:none; } }
  /* Cabecera: texto a la izquierda, publicidad (miniatura del desglose) a la derecha. */
  .hdr-row { display:flex; align-items:center; gap:12px; }
  .hdr-txt { flex:1 1 auto; min-width:0; }
  .hdr-pub { flex:none; display:flex; flex-direction:column; align-items:center; gap:3px; padding:0;
    border:none; background:none; color:#fff; font-family:inherit; cursor:pointer; -webkit-tap-highlight-color:transparent; }
  .hdr-pub:active { opacity:.85; }
  .hdr-row:has(.hdr-pub:not([hidden])) h1 { font-size:17px; }
  .hdr-row:has(.hdr-pub:not([hidden])) .ganado { white-space:normal; overflow:visible; text-overflow:clip; line-height:1.3; }
  .hdr-pub-img { width:40px; height:52px; object-fit:cover; object-position:top; border-radius:7px;
    border:2px solid rgba(255,255,255,.85); background:#fff; box-shadow:0 2px 6px rgba(15,23,42,.25); }
  .hdr-pub-lbl { font-size:9.5px; font-weight:700; letter-spacing:.3px; text-transform:uppercase; opacity:.9; }
  .hdr-pub-monto { font-size:11.5px; font-weight:800; font-variant-numeric:tabular-nums; white-space:nowrap;
    background:#FCD34D; color:#78350F; border-radius:8px; padding:1px 7px; }
  .hdr-pub-monto.al-dia { background:#BBF7D0; color:#14532D; }
  /* Publicidad dentro de la vista 💰 Comisión y en su modal */
  .com-card.pub-card { background:#EFF6FF; border-color:#93C5FD; }
  .pub-card .com-monto { color:#1E40AF; }
  .pub-card.com-pagada { background:#F0FDF4; border-color:#BBF7D0; }
  .pub-mini { width:38px; height:50px; object-fit:cover; object-position:top; border-radius:6px; border:1px solid #BFDBFE; flex:none; background:#fff; }
  .pub-bloque { border:1px solid var(--linea); border-radius:14px; padding:12px; margin-top:12px; background:#F8FAFC; }
  .pub-bloque-head { display:flex; justify-content:space-between; align-items:flex-start; gap:10px; }
  .pub-bloque-tit { font-weight:800; font-size:15px; }
  .pub-bloque-per { color:var(--gris); font-size:13px; margin-top:2px; }
  .pub-chip { font-size:10px; font-weight:700; padding:3px 8px; border-radius:20px; white-space:nowrap; text-transform:uppercase; letter-spacing:.4px; flex:none; }
  ...
  /* Sin ícono 📅 y con el input más angosto: así "Entregado" y "Cobrado" caben en la
     MISMA línea en el celular (antes se bajaban). El date input sigue siendo tocable. */
  .gt-ico { display:none; }
  .fecha-input { font-family:inherit; font-size:13px; padding:6px 5px; border:1px solid var(--linea);
    border-radius:8px; background:#fff; color:var(--tinta); min-height:36px; width:114px; box-sizing:border-box; }
  ...
  .card-wrap.card-nueva { position:relative; margin-top:18px; border-radius:14px;
  ...
  .mes-tit { font-size:13px; font-weight:800; color:var(--gris); text-transform:uppercase;
    letter-spacing:.4px; margin:18px 0 8px; display:flex; justify-content:space-between;
    align-items:center; gap:10px; }
  .mes-filtro { display:flex; gap:6px; margin:0 0 10px; }
  .mes-filtro button { flex:1 1 0; padding:9px 8px; border:1px solid var(--linea); background:#fff;
    color:var(--gris); font-family:inherit; font-size:13px; font-weight:700; border-radius:9px;
    cursor:pointer; min-height:40px; }
  .mes-filtro button.on { background:var(--azul); color:#fff; border-color:var(--azul); }
  .mes-filtro button.on.falta { background:#B45309; border-color:#B45309; }
...
(function () {
  ...
  // Publicidad (tabla cobro_publicidad): Alejandro paga los avisos y al repartidor
  // le toca una parte. Se cobra por el MISMO flujo que las comisiones; en la
  // selección viajan como 'pub:<id>' para no chocar con los ids de entregas.
  var PUB = {};          // id -> fila de cobro_publicidad
  var pubPorPagar = [];  // 'pub:<id>' pendientes (se recalcula en renderComision)
  var CARGADO = false;   // true cuando llegaron los datos vivos (o el caché): recién ahí se pintan cifras en la cabecera
  var CACHE_ENT = 'entregas_cache_v1', CACHE_EST = 'estado_cache_v1', CACHE_PUB = 'publicidad_cache_v1';
  function cacheGet(k) { try { return JSON.parse(localStorage.getItem(k) || 'null'); } catch (e) { return null; } }
  function cacheSet(k, v) { try { localStorage.setItem(k, JSON.stringify(v)); } catch (e) { /* sin espacio o modo privado */ } }
  function esPub(id) { return String(id).indexOf('pub:') === 0; }
  function pubIdDe(id) { return String(id).slice(4); }
  function pubActivos() {
    return Object.keys(PUB).filter(function (id) { return !PUB[id].eliminado; })
      .sort(function (a, b) { return String(PUB[b].hasta || '').localeCompare(String(PUB[a].hasta || '')); });
  }
  function pubPendientes() { return pubActivos().filter(function (id) { return !PUB[id].pagado; }); }
  function pubPeriodo(pb) {
    var d = String(pb.desde || ''), h = String(pb.hasta || '');
    return (d && h) ? (fechaLarga(d) + ' al ' + fechaLarga(h)) : (h || d);
  }
  function pubPeriodoCorto(pb) {
    var d = String(pb.desde || ''), h = String(pb.hasta || '');
    if (!d || !h) { return h || d; }
    var fd = fechaLarga(d), fh = fechaLarga(h);
    if (d.slice(0, 4) === h.slice(0, 4)) { fd = fd.replace(/\s*\d{4}$/, ''); }
    return fd + ' al ' + fh;
  }
  function pubPorc(pb) { var f = Number(pb.fraccion); return isFinite(f) && f > 0 ? Math.round(f * 100) + '%' : 'tu parte'; }
  // Monto y etiqueta de un ítem seleccionable (comisión de entrega o publicidad).
  function montoSel(id) {
    if (esPub(id)) { var pb = PUB[pubIdDe(id)]; return pb ? (Number(pb.monto) || 0) : 0; }
    return META[id] ? META[id].comision : 0;
  }
  function labelSel(id) {
    if (esPub(id)) { var pb = PUB[pubIdDe(id)] || {}; return 'Publicidad ' + (pb.campana || '') + ' (' + pubPorc(pb) + ')'; }
    return (META[id] || {}).cliente || id;
  }
  ...
  function aplicarCobroCompacto(card, id, entregado, cobrado, esServ) {
    ...
    // Chip "💰 Falta cobrar" centrado ARRIBA, montado sobre el borde de la card
    // (pedido de Alejandro): se ve el estado de un vistazo sin leer el cuerpo.
  ...
  function seleccionadas() { return comPorPagar.concat(pubPorPagar).filter(function (id) { return !comDesel.has(id); }); }
  ...
  var mesFiltro = 'todas'; // 'todas' | 'falta'
  ...
  function renderMes() {
    ...
    // Una sola lista con filtro, no dos: antes lo pendiente salía dos veces (marcado en
    // ámbar arriba y repetido abajo en «lo que falta cobrar»), y leyendo el panel parecía
    // que había el doble de entregas por cobrar de las que hay.
    var verSoloFalta = (mesFiltro === 'falta') && pendientes.length > 0;
    var mostrar = verSoloFalta ? pendientes : ids;
    ...
    if (pendientes.length) {
      html += '<div class="mes-filtro">'
        + '<button type="button" data-filtro="todas"' + (verSoloFalta ? '' : ' class="on"') + '>'
        + 'Todas · ' + ids.length + '</button>'
        + '<button type="button" data-filtro="falta"' + (verSoloFalta ? ' class="on falta"' : '') + '>'
        + 'Por cobrar · ' + pendientes.length + '</button>'
        + '</div>';
    }
    html += '<div class="mes-tit"><span>' + mostrar.length + ' entregas</span>'
      + '<span>' + banos + ' baños en el mes</span></div>'
      + '<div class="mes-lista">';
    ...
    if (!mostrar.length) { html += '<p class="mes-vacio">No hay entregas en este mes.</p>'; }
    html += '</div>';
    cont.innerHTML = html;

    cont.querySelectorAll('.mes-filtro button').forEach(function (b) {
      b.addEventListener('click', function () {
        mesFiltro = b.getAttribute('data-filtro');
        renderMes();
      });
    });
  }
  ...
  function actualizarGanado() {
    ...
    // Mientras no lleguen los datos vivos, la cabecera muestra el esqueleto: las cifras
    // horneadas pueden ser de hace semanas (33 pendientes que ya no existen).
    if (!CARGADO) { setHeaderH(); return; }
  ...
  function horaCorta(d) { var h = d.getHours(), m = d.getMinutes(); return (h < 10 ? '0' : '') + h + ':' + (m < 10 ? '0' : '') + m; }

  // ====================== PUBLICIDAD (cobro_publicidad) ======================
  function upsertPub(id, patch) {
    var prev = PUB[id] || {};
    var row = {};
    for (var k in prev) { row[k] = prev[k]; }
    for (var q in patch) { row[q] = patch[q]; }
    PUB[id] = row;
    renderHeaderPub(); renderComision();
    if (!SUPA.url || !SUPA.key) return Promise.resolve(true);
    return fetch(SUPA.url + '/rest/v1/cobro_publicidad?id=eq.' + encodeURIComponent(id), {
      method: 'PATCH',
      ...
      if (!r.ok) { throw new Error('HTTP ' + r.status); }
      bannerOnline(true); return true;
    }).catch(function (e) { console.warn('No se pudo guardar publicidad', e); bannerOnline(false); return false; });
  }

  // Miniatura + monto pendiente a la derecha de la cabecera. Sin cobros, no aparece.
  function renderHeaderPub() {
    var btn = document.getElementById('hdr-pub');
    if (!btn) { return; }
    var ids = pubActivos();
    if (!ids.length) { btn.hidden = true; setHeaderH(); return; }
    var pend = pubPendientes();
    var muestra = PUB[pend[0] || ids[0]];
    var img = btn.querySelector('.hdr-pub-img');
    var src = muestra.miniatura || muestra.imagen || '';
    if (img && src && img.getAttribute('src') !== src) {
      img.src = src;
      img.onerror = function () { if (muestra.imagen && img.src.indexOf(muestra.imagen) === -1) { img.src = muestra.imagen; } };
    }
    var total = pend.reduce(function (t, id) { return t + (Number(PUB[id].monto) || 0); }, 0);
    var m = btn.querySelector('.hdr-pub-monto');
    if (m) {
      if (pend.length) { m.textContent = clp(total); m.classList.remove('al-dia'); }
      else { m.textContent = 'Al día ✓'; m.classList.add('al-dia'); }
    }
    btn.hidden = false;
    setHeaderH();
  }
  ...
  function irAVista(v) {
    var b = document.querySelector('.vista-btn[data-vista="' + v + '"]');
    if (b) { b.click(); }
  }

  function bloquePub(id, conBoton) {
    var pb = PUB[id] || {};
    var det = pb.detalle || {};
    var lineas = '';
    if (det.clics != null) { lineas += '<div class="bank-row"><span>Personas que entraron al aviso</span><b>' + escapeHtml(String(det.clics)) + '</b></div>'; }
    if (det.contactos != null) { lineas += '<div class="bank-row"><span>Personas que contactaron</span><b>' + escapeHtml(String(det.contactos)) + '</b></div>'; }
    if (det.costo_por_contacto) { lineas += '<div class="bank-row"><span>Costo por cada contacto</span><b>' + clp(det.costo_por_contacto) + '</b></div>'; }
    var chip = pb.pagado
    ...
    return '<div class="pub-bloque">'
      ...
      + '<div class="pub-bloque-per">' + escapeHtml(pb.plataforma || '') + ' · ' + escapeHtml(pubPeriodo(pb)) + '</div></div>' + chip + '</div>'
      + '<div class="bank"><div class="bank-row"><span>Total gastado en publicidad</span><b>' + clp(pb.gasto_total) + '</b></div>'
      + lineas
      + '<div class="bank-row bank-total"><span>Tu parte (' + escapeHtml(pubPorc(pb)) + ')</span><b>' + clp(pb.monto) + '</b></div></div>'
      + (pb.nota ? '<p class="modal-sub">' + escapeHtml(pb.nota) + '</p>' : '')
      + boton + imgHtml + '</div>';
  }
  ...
  function abrirModalPublicidad() {
    ...
    bg.innerHTML = '<div class="modal"><h3>📣 Publicidad</h3>'
      + '<p class="modal-sub">Alejandro paga los avisos que traen clientes; a ti te toca una parte. '
      + (pend.length ? 'Pendiente por pagar: <b>' + clp(tot) + '</b>.' : 'Estás al día. ✓') + '</p>'
      + ids.map(function (id) { return bloquePub(id, true); }).join('')
      + '<div class="modal-acciones"><button type="button" class="btn-modal cancelar">Cerrar</button></div></div>';
    document.body.appendChild(bg);
    function cerrar() { if (document.body.contains(bg)) { document.body.removeChild(bg); } }
    bg.addEventListener('click', function (ev) { if (ev.target === bg) { cerrar(); } });
    bg.querySelector('.cancelar').addEventListener('click', cerrar);
    bg.querySelectorAll('.pub-ir').forEach(function (b) {
      b.addEventListener('click', function () {
        comDesel.delete('pub:' + b.getAttribute('data-id')); saveSel();
        cerrar(); irAVista('comision'); renderComision();
      });
    });
  }
  ...
  function renderComision() {
    ...
    var pubPend = pubPendientes(), pubPag = pubActivos().filter(function (id) { return PUB[id].pagado; });
    pubPorPagar = pubPend.map(function (id) { return 'pub:' + id; });

    var deudaCom = porPagar.reduce(function (s, id) { return s + META[id].comision; }, 0);
    var deudaPub = pubPend.reduce(function (s, id) { return s + (Number(PUB[id].monto) || 0); }, 0);
    var deuda = deudaCom + deudaPub;
    var yaPagada = pagadas.reduce(function (s, id) { return s + META[id].comision; }, 0)
      + pubPag.reduce(function (s, id) { return s + (Number(PUB[id].monto) || 0); }, 0);

    // Tarjeta de un cobro de publicidad (misma silueta que la comisión: check, texto, monto).
    function cardPub(id, paid) {
      var pb = PUB[id] || {}, sid = 'pub:' + id;
      var check = paid
        ? '<span class="com-pagada-tag">✓ Pagada</span>'
        : '<input type="checkbox" class="com-check" data-id="' + escapeHtml(sid) + '"' + (comDesel.has(sid) ? '' : ' checked') + '>';
      var rev = paid ? '<button type="button" class="com-revocar" data-id="' + escapeHtml(sid) + '">Revocar</button>' : '';
      var mini = (pb.miniatura || pb.imagen) ? '<img class="pub-mini" alt="" src="' + escapeHtml(pb.miniatura || pb.imagen) + '">' : '';
      return '<div class="com-card pub-card' + (paid ? ' com-pagada' : '') + '">'
        + '<div class="com-top">' + check + mini
        + '<div class="com-main"><div class="com-cli">📣 Publicidad · ' + escapeHtml(pb.campana || '') + '</div>'
        + '<div class="com-dir">' + escapeHtml(pubPeriodoCorto(pb)) + ' · ' + (Number(pb.fraccion) === 0.5 ? 'la mitad de ' : escapeHtml(pubPorc(pb)) + ' de ') + clp(pb.gasto_total) + '</div></div>'
        + '<div class="com-monto">' + clp(pb.monto) + '</div>' + rev + '</div>'
        + '<div class="com-acciones"><button type="button" class="com-info pub-info" data-id="' + escapeHtml(id) + '">🧾 Ver el desglose</button></div>'
        + '</div>';
    }
    ...
    var deudaTxt = (pubPend.length && porPagar.length) ? 'Te deben (comisiones + publicidad)'
      : (pubPend.length ? 'Te deben (publicidad)' : 'Te deben (cobradas sin pagar)');
    var html = '<div class="cp-head"><span class="cp-titulo">💰 Comisión a pagar</span></div>'
      + '<div class="com-deuda"><span>' + deudaTxt + '</span><b>' + clp(deuda) + '</b></div>';
    if (pubPend.length) {
      html += '<div class="com-sec-tit">📣 Publicidad · pendiente por pagar</div>'
        + pubPend.map(function (id) { return cardPub(id, false); }).join('');
    }
    if (porPagar.length) {
      html += '<div class="com-sec-tit">' + (pubPend.length ? 'Comisiones · selecciona las que vas a pagar' : 'Selecciona las que vas a pagar') + '</div>'
        + porPagar.map(function (id) { return card(id, false); }).join('');
    } else if (!pubPend.length) {
      html += '<p class="cp-vacio">No hay comisiones pendientes de pago. 🎉</p>';
    }
    if (pagadas.length || pubPag.length) {
      // Agrupar por momento de pago: cada grupo es un "Pago N" con su comprobante.
      // La publicidad pagada entra al grupo de su pagada_at (mismo pago, misma foto).
      var grupos = {};
      pagadas.forEach(function (id) {
        var k = pagadaAtDe(id) || 'sin-fecha';
        (grupos[k] = grupos[k] || []).push(id);
      });
      pubPag.forEach(function (id) {
        var k = PUB[id].pagada_at || 'sin-fecha';
        (grupos[k] = grupos[k] || []).push('pub:' + id);
      });
      ...
      claves.forEach(function (k, i) {
        var idsg = grupos[k];
        var tot = idsg.reduce(function (s, id) { return s + montoSel(id); }, 0);
        ...
        html += '<div class="pago-grupo"><div class="pago-head">'
          ...
          + idsg.map(function (id) { return esPub(id) ? cardPub(pubIdDe(id), true) : card(id, true); }).join('')
          + '</div>';
      });
    }
    cont.innerHTML = html;
    ...
    cont.querySelectorAll('.com-revocar').forEach(function (b) {
      b.addEventListener('click', function () {
        ...
        if (esPub(rid)) {
          if (confirm('¿Revocar el pago de esta publicidad? Volverá a quedar como pendiente.')) {
            upsertPub(pubIdDe(rid), { pagado: false, pagada_at: null });
          }
          return;
        }
        if (confirm('¿Revocar el pago de esta comisión? Volverá a quedar como pendiente.')) {
          upsert(rid, { comision_pagada: false, pagada_at: null });
        }
      });
    });
  ...
  function seleccionTotal() {
    return seleccionadas().reduce(function (t, id) { return t + montoSel(id); }, 0);
  }
  ...
  function abrirModalPago() {
    ...
    var total = seleccionTotal();
    var detalle = seleccion.map(function (id) { return '<div class="bank-row"><span>' + escapeHtml(labelSel(id)) + '</span><b>' + clp(montoSel(id)) + '</b></div>'; }).join('');
    var hayPub = seleccion.some(esPub), hayCom = seleccion.some(function (id) { return !esPub(id); });
    var titulo = hayPub && hayCom ? 'Pagar comisión y publicidad' : (hayPub ? 'Pagar publicidad' : 'Pagar comisión');
    var bg = document.createElement('div');
    bg.className = 'modal-bg';
    bg.innerHTML =
      '<div class="modal"><h3>' + titulo + '</h3>'
    ...
    bg.querySelector('.confirmar').addEventListener('click', function () {
      ...
      Promise.all(seleccion.map(function (id) {
          return esPub(id) ? upsertPub(pubIdDe(id), { pagado: true, pagada_at: ahora })
                           : upsert(id, { comision_pagada: true, pagada_at: ahora });
        }))
        .then(function (oks) {
          ...
          oks.forEach(function (ok, i) {
            if (ok) { return; }
            fallo = true; var id = seleccion[i];
            if (esPub(id)) { var pb = PUB[pubIdDe(id)]; if (pb) { pb.pagado = false; pb.pagada_at = null; } }
            else if (estado[id]) { estado[id].comision_pagada = false; estado[id].pagada_at = null; }
          });
          if (fallo) {
            renderHeaderPub(); renderComision();
          ...
          var lineas = seleccion.map(function (id) { return '• ' + labelSel(id) + ': ' + clp(montoSel(id)); });
          var texto = 'Hola Alejandro, te transfiero ' + (hayPub && hayCom ? 'la comisión y la publicidad' : (hayPub ? 'mi parte de la publicidad' : 'la comisión')) + ':\n' + lineas.join('\n')
  ...
  function metaDeData(e) {
    ...
    // datos VIVOS de Supabase y pisa lo horneado, así que si no conoce la regla, un monto
    // fijo vuelve al 20% en cuanto el panel refresca (pasó con American Container: la
    // ficha decía $874.000 y el panel mostraba $1.520.000).
    var fija = (typeof e.comision === 'number') ? e.comision : null;
    var comision = !comisiona ? 0
      : (fija != null ? Math.round(fija) : Math.round(neto * 0.20));
    return {
  ...
  function load() {
    var done = function () { CARGADO = true; pintarTodo(); reubicarCards(); actualizarProgreso(); aplicarAnteriores(); aplicarTareas(); renderHeaderPub(); renderComision(); revelar(); irAlAncla(); };
    if (!SUPA.url || !SUPA.key) { setActualizado('sin conexión a la base'); done(); return; }
    // Estado mutable (entregado/cobrado/…): puebla el mapa `estado`. null = no llegó.
    ...
    var pEntrega = fetch(SUPA.url + '/rest/v1/entrega?select=id,fecha,informado_at,data,card_html&eliminado=eq.false&order=informado_at.desc', { headers: headers() })
      .then(function (r) { return r.ok ? r.json() : null; })
      .catch(function (e) { console.warn(e); return null; });
    // Cobros de publicidad (la parte que le toca al repartidor). Si falla, simplemente no se muestra.
    var pPub = fetch(SUPA.url + '/rest/v1/cobro_publicidad?select=*&eliminado=eq.false&order=hasta.desc', { headers: headers() })
      .then(function (r) { return r.ok ? r.json() : null; })
      .catch(function (e) { console.warn(e); return null; });
    Promise.all([pEstado, pEntrega, pPub]).then(function (res) {
      var est = res[0], rows = res[1], pubs = res[2];
      var vivo = true, horneado = false;
      // Estado: vivo → guarda caché; si no llegó, usa el de la última visita (si hay).
      if (est) { cacheSet(CACHE_EST, est); }
      else { vivo = false; est = cacheGet(CACHE_EST) || []; }
      est.forEach(function (row) { estado[row.id] = row; });
      // Entregas: vivo → guarda caché; si no llegó, el caché de la última visita; y si
      // tampoco hay, quedan las horneadas del HTML (respaldo de último recurso).
      if (rows && rows.length) { cacheSet(CACHE_ENT, rows); }
      else { vivo = false; rows = cacheGet(CACHE_ENT); if (!rows || !rows.length) { horneado = true; } }
      if (rows && rows.length) {
        try { aplicarEntregas(rows); }
        catch (e) { console.warn('No se pudo aplicar entregas de Supabase, uso las horneadas:', e); horneado = true; }
      }
      if (pubs) { cacheSet(CACHE_PUB, pubs); } else { pubs = cacheGet(CACHE_PUB) || []; }
      pubs.forEach(function (pb) { if (pb && pb.id) { PUB[pb.id] = pb; } });
      setActualizado(vivo ? 'Actualizado ' + horaCorta(new Date()) : (horneado ? 'Sin conexión · respaldo antiguo' : 'Sin conexión · lo último guardado'));
      if (!vivo) {
        var p = document.getElementById('estado-online');
        if (p) {
          p.hidden = false;
          p.textContent = horneado
            ? '⚠ Sin conexión con la base: estás viendo un respaldo ANTIGUO. Cuando vuelva la señal, recarga.'
            : '⚠ Sin conexión con la base: estás viendo lo último guardado en este teléfono.';
        }
      }
      done();
    });
  ...
  function revelar() {
    var m = document.querySelector('main'); if (m) { m.classList.add('listo'); }
    document.body.classList.add('listo');
    var sk = document.getElementById('skel'); if (sk) { sk.hidden = true; }
    setHeaderH();
  }
  ...
  <header>
    <div class="hdr-row">
      <div class="hdr-txt">
        ...
        <div class="sub"><span id="pend-count"><span class="skel skel-n"></span></span> pendiente(s) · <span id="hdr-act">Cargando…</span></div>
        <div class="sub ganado" id="ganado-total">💰 <span class="skel skel-w"></span></div>
      </div>
      <button type="button" id="hdr-pub" class="hdr-pub" hidden aria-label="Ver el desglose de la publicidad">
        <img class="hdr-pub-img" alt="">
        <span class="hdr-pub-lbl">📣 Publicidad</span>
        <span class="hdr-pub-monto"></span>
      </button>
    </div>
  </header>
  <div id="skel" aria-hidden="true">
    <div class="skel-tabs"><div class="skel-tab"></div><div class="skel-tab"></div></div>
    <div class="skel-tit"></div>
